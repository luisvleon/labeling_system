...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  ...
  $insertSQL = sprintf("INSERT INTO tbl_inv_simple (invoice_id, box, modelo, des_id, qty, marca_id, ori_id, size_id, material, precio, costo, pvp, user_id, pieces) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
					   ...
					   GetSQLValueString($_POST['select_sizes'], "int"),
  ...
  mysql_select_db($database_labels_con, $labels_con);
$query_tallas = "SELECT * FROM tbl_settings WHERE parameter = 'create_label'";
...
if ($row_tallas['value']= 1) { 
 ...
 //pass variables
  ...
  setcookie("size", $_POST['select_sizes']);
  setcookie("qty", $_POST['txt_qty'] * $_POST['text_pieces']);
    
  //$insertGoTo = "create_labels.php?inv_id=" . $_POST['hidden_inv'];
...
if (isset($_COOKIE["inv_id"])) {
  $colname_invoice = $_COOKIE["inv_id"];
}
mysql_select_db($database_labels_con, $labels_con);
...
if (isset($_COOKIE["inv_id"])) {
  $colname_inv_simple = $_COOKIE["inv_id"];
}
mysql_select_db($database_labels_con, $labels_con);
$query_inv_simple = sprintf("SELECT inv.*, des.des_text, mar.marca_text, ori.ori_name, siz.size_label FROM tbl_inv_simple inv, tbl_description des, tbl_marcas mar, tbl_origin ori, tbl_sizes siz WHERE inv.size_id = siz.size_id AND inv.marca_id=mar.marca_id and inv.des_id=des.des_id and inv.ori_id=ori.ori_id and inv.invoice_id = %s ORDER BY inv.inv_id DESC LIMIT 100 ", GetSQLValueString($colname_inv_simple, "int"));
...
mysql_select_db($database_labels_con, $labels_con);
$query_inv_total = "SELECT SUM(`qty` * `precio`) as totales FROM `tbl_inv_simple` WHERE invoice_id =" . $_GET['invoice_id'] . "";
$inv_total = mysql_query($query_inv_total, $labels_con) or die(mysql_error());
$row_inv_total = mysql_fetch_assoc($inv_total);
$totalRows_inv_total = mysql_num_rows($inv_total);
...
<table width="980" border="0" align="center">
    ...
    <td width="814" height="652" valign="top"><!-- InstanceBeginEditable name="edit1" --><br />
      ...
      <table width="779" border="1" align="center">
        <tr>
          ...
          <td colspan="4"><?php echo str_pad((int) ($row_invoice['invoice_id']),6,"0",STR_PAD_LEFT); ?></td>
          <td>Total $</td>
          <td><?php echo $row_inv_total['totales']; ?></td>
        </tr>
        <tr>
          <td bgcolor="#333333" class="cabecera">Company:</td>
          <td colspan="6"><?php echo $row_invoice['cos_corpname']; ?>  <?php echo $row_invoice['cos_corpname2']; ?></td>
        </tr>
        <tr>
          <td bgcolor="#333333" class="cabecera">Etiquetas</td>
          <td width="123"><?php echo $row_etiqtotal['SUM(`qty` * `pieces`)']; ?></td>
          <td width="123" >Items: <?php echo $row_etiqtotal['SUM(`qty` * 1)']; ?></td>
          <td width="60">&nbsp;</td>
          <td width="61">Print: <?php echo $row_total['total']; ?></td>
          <td width="131" bgcolor="#333333" class="cabecera">Total facturar</td>
          <td width="120"><?php echo (($row_etiqtotal['SUM(`qty` * `pieces`)']) * $row_invoice['precio']) - $row_invoice['anticipo']; ?></td>
        </tr>
      </table>
      <br />
  ...
  <table width="1064" border="1" align="center">
    <tr>
      ...
      <td colspan="3"><input name="txt_modelo" type="text" class="cajagrande" id="txt_modelo" size="13" maxlength="30" /></td>
      <td colspan="2">Qty:
        <input name="txt_qty" type="text" class="cajagrande" id="txt_qty" size="4" maxlength="4" autocomplete="off" /></td>
      <td width="189">Pieces: 
        <label>
...
?>
      ...
      <td>Origin:</td>
      <td colspan="4"><select name="select_origen" class="cajagrande" id="select_origen">
        <?php
do {  
?>
...
?>
      ...
      <td width="127" align="center"><a href="/labels/inv_simple2.php?invoice_id=<?php echo $row_invoice['invoice_id']; ?>&orden=inv.inv_id" target="_blank">Ver todo</a></td>
      <td align="center"><a href="/labels/prices_search.php?id=<?php echo $row_invoice['cos_id']; ?>" target="_blank">
        </a><label for="select_sizes"></label>
        <select name="select_sizes" class="cajagrande_mediana" id="select_sizes">
          <?php
do {  
?>
...
?>
    ...
    <tr>
      <td>Components:</td>
      <td colspan="8"><input name="txt_mat1" type="text" class="cajagrande" id="txt_mat1" size="6" maxlength="30" autocomplete="off"/>
        %
...
?>
      ...
      <td width="187" align="center"><a href="prices_search.php?id=<?php echo $row_invoice['cos_id']; ?>" target="_blank">Prices</a></td>
      <td width="113">Unit Price:
        <input name="txt_precio" type="text" class="cajagrande" id="txt_precio" value="0" size="6" maxlength="6" autocomplete="off" /></td>
      <td colspan="2">Cost:
        <input name="text_costo" type="text" class="cajagrande" id="text_costo" value="0" size="6" maxlength="5" autocomplete="off" /></td>
      <td>PVP
...
<br />
      ...
      <table width="1310" border="1" align="center">
  ...
  <tr bgcolor="#333333" class="cabecera">
    ...
    <td width="135" align="center">Talla</td>
  ...
  <?php do { ?>
    <tr>
      ...
      <td><a href="/labels/inv_simple_clone.php?inv_id=<?php echo $row_inv_simple['inv_id']; ?>&amp;invoice_id=<?php echo $row_inv_simple['invoice_id']; ?>"><?php echo $row_inv_simple['modelo']; ?></a></td>
      <td align="center"><?php echo $row_inv_simple['size_label']; ?></td>
