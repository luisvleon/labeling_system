...
mysql_select_db($database_labels_con, $labels_con);
$query_prices = sprintf("SELECT DISTINCT sim.inv_id, sim.precio, sim.costo, cos.cos_names, mar.marca_text, des.des_text  FROM tbl_inv_simple sim, tbl_marcas mar, tbl_description des, tbl_invoice inv, tbl_costumers cos WHERE inv.activo = 1 AND inv.cos_id = %s AND sim.marca_id=". $_POST['select_brand'] . " AND sim.marca_id = mar.marca_id AND sim.des_id=" . $_POST['select_descrip'] . " AND sim.des_id = des.des_id AND sim.invoice_id = inv.invoice_id AND inv.cos_id = cos.cos_id", 
...
?>
  ...
  <table width="755" border="1" align="center">
    ...
    <?php do { ?>
      <tr>
        <td><?php echo $row_prices['inv_id']; ?></td>
