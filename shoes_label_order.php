...
$capellada_var = "";
if ($_POST['txt_capellada'] <> "") { $capellada_var = $_POST['txt_capellada'] . "% " . $_POST['select_capellada']; }
if ($_POST['txt_capellada2'] <> "") {$capellada_var = $capellada_var . " " . $_POST['txt_capellada2'] . "% " . $_POST['select_capellada2']; } else {  }

$forro_var = "";
if ($_POST['txt_forro'] <> "") { $forro_var = $_POST['txt_forro'] . "% " . $_POST['select_forro']; }

$plantilla_var = "";
if ($_POST['txt_plantilla'] <> "") { $plantilla_var = $_POST['txt_plantilla'] . "% " . $_POST['select_plantilla']; }

$suela_var = "";
if ($_POST['txt_suela'] <> "") { $suela_var = $_POST['txt_suela'] . "% " . $_POST['select_suela']; }
if ($_POST['txt_suela2'] <> "") { $suela_var = $suela_var . " " . $_POST['txt_suela2'] . "% " . $_POST['select_suela2']; } else { }
	
  $insertSQL = sprintf("INSERT INTO tbl_shoes_labels (costumer, invoice_id, estilo, size, marca, capellada_text, forro_text, plantilla_text, zuela_text, hiddelavado, origin, `user`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                       GetSQLValueString($_POST['hidden_cos'], "int"),
                       GetSQLValueString($_REQUEST['inv'], "text"),
					   GetSQLValueString($_POST['txt_estilo'], "text"),
					   GetSQLValueString($_POST['txt_size'], "text"),
					   GetSQLValueString($_POST['select_marca'], "int"),
					   GetSQLValueString($capellada_var, "text"),
                       GetSQLValueString($forro_var, "text"),
                       GetSQLValueString($plantilla_var, "text"),
                       GetSQLValueString($suela_var, "text"),
					   GetSQLValueString($_POST['select_show'], "int"),
  ...
  $insertGoTo = "shoes_label_qty.php?id=" . ($_POST['hidden_maximo'] + 1) . "&inv=" . $_REQUEST['inv'];
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_labels_con, $labels_con);
$query_marcas = "SELECT mar.* FROM tbl_marcas mar, tbl_costumers cos WHERE mar.cos_id = cos.cos_id  AND mar.cos_id = " . $_REQUEST["cos"] ." ORDER BY mar.marca_text ASC";
  ...
  <tr>
    <td width="814" height="652" valign="top"><!-- InstanceBeginEditable name="edit1" -->
      <table width="537" border="0" align="center">
        <tr>
          <td width="531" align="center" class="Titulo">Order shoes labels</td>
        </tr>
      </table>
      <br />
      <table width="922" height="198" border="0" align="center">
        <tr>
...
do {  
?>
                    <option value="<?php echo $row_ori['ori_id']?>"><?php echo $row_ori['ori_name']?></option>
                    <?php
...
?>
              ...
              <tr>
                ...
                <td width="421"><label>
                  <input name="txt_estilo" type="text" class="cajagrande" id="txt_estilo" />
                </label>
                  <label for="txt_size"></label>
                  <label>
                  </label></td>
                <td width="49"><span class="cabecera">Size:</span></td>
                <td width="219"><input name="txt_size" type="text" class="cajagrandeSIN" id="txt_size" size="8" maxlength="20" /></td>
              </tr>
              <tr>
...
?>
              ...
              <tr>
                ...
                <td colspan="3"><label>
                  <input name="txt_capellada" type="text" class="cajagrande" id="txt_capellada" value="100" size="5" maxlength="3" />
                  % 
                  <select name="select_capellada" class="cajagrande" id="select_capellada">
                    <?php
do {  
?>
                    <option value="<?php echo $row_cape['mat_text']?>"><?php echo $row_cape['mat_text']?></option>
                    <?php
} while ($row_cape = mysql_fetch_assoc($cape));
  $rows = mysql_num_rows($cape);
  if($rows > 0) {
      mysql_data_seek($cape, 0);
	  $row_cape = mysql_fetch_assoc($cape);
  }
?>
                  </select>
                  - 
                  <input name="txt_capellada2" type="text" class="cajagrande" id="txt_capellada2" size="5" maxlength="3" />
                  %
                  <select name="select_capellada2" class="cajagrande" id="select_capellada2">
                    <?php
do {  
?>
                    <option value="<?php echo $row_cape['mat_text']?>"><?php echo $row_cape['mat_text']?></option>
...
?>
              ...
              <tr>
                ...
                <td colspan="3"><label>
                  <input name="txt_forro" type="text" class="cajagrande" id="txt_forro" value="100" size="5" maxlength="3" />
                  % 
                  <select name="select_forro" class="cajagrande" id="select_forro">
                    <?php
do {  
?>
                    <option value="<?php echo $row_forro['text_forro']?>"><?php echo $row_forro['text_forro']?></option>
                    <?php
} while ($row_forro = mysql_fetch_assoc($forro));
  $rows = mysql_num_rows($forro);
  if($rows > 0) {
      mysql_data_seek($forro, 0);
	  $row_forro = mysql_fetch_assoc($forro);
  }
?>
                  </select>
              ...
              <tr>
                ...
                <td colspan="3"><label>
                  <input name="txt_plantilla" type="text" class="cajagrande" id="txt_plantilla" value="100" size="5" maxlength="3" />
%
<select name="select_plantilla" class="cajagrande" id="select_plantilla">
  <?php
do {  
?>
  <option value="<?php echo $row_plan['text_plan']?>"><?php echo $row_plan['text_plan']?></option>
  <?php
...
?>
              ...
              <tr>
                ...
                <td colspan="3"><label>
                  <input name="txt_suela" type="text" class="cajagrande" id="txt_suela" value="100" size="5" maxlength="3" />
%
                  <select name="select_suela" class="cajagrande" id="select_suela">
                    <?php
do {  
?>
                    <option value="<?php echo $row_zuela['text_zuela']?>"><?php echo $row_zuela['text_zuela']?></option>
                    <?php
} while ($row_zuela = mysql_fetch_assoc($zuela));
  $rows = mysql_num_rows($zuela);
  if($rows > 0) {
      mysql_data_seek($zuela, 0);
	  $row_zuela = mysql_fetch_assoc($zuela);
  }
?>
                  </select>
                - 
                <input name="txt_suela2" type="text" class="cajagrande" id="txt_suela2" size="5" maxlength="3" />
                %
                <select name="select_suela2" class="cajagrande" id="select_suela2">
                  <?php
do {  
?>
                  <option value="<?php echo $row_zuela['text_zuela']?>"><?php echo $row_zuela['text_zuela']?></option>
...
?>
              ...
              <tr>
                <td height="46" bgcolor="#333333" class="cabecera"><input name="hidden_user" type="hidden" id="hidden_user" value="<?php echo $row_Recordset1['user_id']; ?>" />
                  ...
                  <input name="hidden_inv" type="hidden" id="hidden_inv" value="<?php echo $row_Recordset1['user_names']; ?>" /></td>
                <td colspan="3"><label>
...
<script type="text/javascript">
function setFocus(){
document.getElementById("select_origin").focus();
}
</script>
</body>
<!-- InstanceEnd -->
